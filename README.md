...
Harness the prowess of OpenAI-compatible APIs to seamlessly interface with Amazon Bedrock. Erected upon the foundation of the [Bedrock Access Gateway](https://github.com/aws-samples/bedrock-access-gateway/tree/main)，this augmented solution introduces the empowering capability of observability. Enriched with monitoring and alerting functionalities, coupled with the automated scaling of Fargate services, this architectural paradigm elevates the stability and responsiveness of the overarching system to unprecedented heights.

## Breaking Changes

To adhere to security best practices, this solution now uses Secrets Manager to manage API keys. You must first create an API key in Secrets Manager and regularly rotate this key. This update introduces monitoring alert functions and Fargate service auto-scaling, enhancing the stability and responsiveness of the solution.

If you still encounter issues, please raise a GitHub issue.
...
- [x] Support ECS Service Auto Scaling (**new**)
- [x] Support Cloudwatch Container Insights (**new**)
- [x] Support Cloudwatch Alarms (**new**)
- [x] Support Enable ALB Access logs (**new**)
...
2. Click the following button to launch the CloudFormation Stack in that region. Choose one of the following:
  ...
  - **ALB + Lambda**
      ...
      Please use the deployment/BedrockProxyWithObservability.template file.
   - **ALB + Fargate**

      [![Launch Stack](assets/launch-stack.png)]

      Please use the deployment/BedrockProxyFargateWithObservability.template file.
...
4. On the "Specify stack details" page, provide the following information:
  ...
  4-1. Use Lambda deploy mode

   ![Lambda Deploy](assets/Lambda-deploy.png)

    EnableALBAccessLog: Set this to true if you want to enable access logs for the Application Load Balancer, it defaults to false. Click "Next" to proceed.

  4-2. Use Fargate deploy mode

   ![Fargate Deploy](assets/Fargate-deploy.png)

    EnableCloudWatchAlarms: set the value to true, as it defaults to false. Proceed by clicking "Next". If set to true, configure the SNSTopicArn; otherwise, leave it blank. Ensure that the SNS topic is pre-created and subscriptions are properly configured.
...
### Are there plans to support Lambda Insights?

Currently, there are no plans to support Lambda Insights. It depends on whether there is customer demand.

### Are there plans to support pre-provisioned LCUs for ALBs to handle burst traffic scenarios?

At present, there are no plans to support pre-provisioned LCUs for ALBs. This is contingent upon customer demand.

### Are there plans to support Lambda Reserve Concurrency to handle burst traffic scenarios?

There are currently no plans to support Lambda Reserve Concurrency. This will be determined by customer demand.
